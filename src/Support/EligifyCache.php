...
class EligifyCache
{
    ...
    public function rememberEvaluation(Criteria $criteria, array|Snapshot $data, callable $callback): array
    {
        ...
        if ($this->supportsTags()) {
            return Cache::tags(['eligify_evaluations', $this->getCriteriaTag($criteria)])
    ...
    public function rememberCompilation(Criteria $criteria, callable $callback): mixed
    {
        ...
        if ($this->supportsTags()) {
            return Cache::tags(['eligify_compilations', $this->getCriteriaTag($criteria)])
                ->remember($key, $ttl, $callback);
        }
    ...
    {
        if ($this->supportsTags()) {
            return Cache::tags([$this->getCriteriaTag($criteria)])->flush();
        }
    ...
    {
        if ($this->supportsTags()) {
            return Cache::tags(['eligify_evaluations'])->flush();
        }
    ...
    {
        if ($this->supportsTags()) {
            return Cache::tags(['eligify_compilations'])->flush();
        }
    ...
    public function flushAllEligifyCache(): bool
    {
        ...
        if ($this->supportsTags()) {
            $flushed = true;
            $flushed = Cache::tags(['eligify_evaluations'])->flush() && $flushed;
            $flushed = Cache::tags(['eligify_compilations'])->flush() && $flushed;
